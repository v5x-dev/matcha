...
>
	...
	<dialog
		open
		class="fixed inset-y-0 right-0 left-auto z-10 m-0 flex h-full w-3/4 max-w-sm flex-col border-l border-border bg-popover text-sm text-popover-foreground shadow-xl transition-transform duration-200 ease-out"
		class:translate-x-0={open}
		class:translate-x-full={!open}
		aria-labelledby="filters-title"
	>
		<div class="relative flex flex-col gap-1.5 p-6">
		...
		<div class="min-h-0 flex-1 overflow-y-auto">
			<div class="flex flex-col gap-5 px-4 pb-4">
				<div class="flex flex-col gap-2">
					...
					<div class="flex flex-wrap" role="group" aria-label="when">
						{#each timeframes as timeframe (timeframe.value)}
							<button
								type="button"
								class={`inline-flex h-8 items-center justify-center border border-border px-2 text-xs font-medium transition-colors first:rounded-l-md last:rounded-r-md hover:bg-accent hover:text-accent-foreground ${draft.timeframe === timeframe.value ? 'bg-primary text-primary-foreground' : 'bg-background'}`}
				...
				<div class="flex flex-col gap-1">
					...
					{#each regions.slice(0, 25) as region (region.value)}
						<button
							type="button"
							class={rowClass}
